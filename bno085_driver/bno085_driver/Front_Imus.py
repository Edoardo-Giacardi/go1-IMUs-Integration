...
class FrontImusPub(Node):
    def __init__(self):
        super().__init__('front_imus_pub')

        # Initialize the two I2C buses
        self.i2c_buses = {
            0: I2C(0),  # /dev/i2c-0 for sensors 1 FL 
            1: I2C(1),  # /dev/i2c-1 for sensors 2 FR
        }
        ...
        try:
            ...
            self.get_logger().error(f"Failed to initialize sensor {i+1}: {e}")
            return
        ...
        self.pub_2imus_front = self.create_publisher(TwoIMU, '/two_imus_front', 10)

        # Create a timer at ~250Hz 
        self.timer_period = 0.004  
        self.timer = self.create_timer(self.timer_period, self.publish_imus)

        self.get_logger().info("FrontImusPublisher node started.")
    ...
    def publish_imus(self):
        ...
        self.pub_2imus_front.publish(msg)
