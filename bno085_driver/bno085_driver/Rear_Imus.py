...
class RearImusPub(Node):
    def __init__(self):
        super().__init__('rear_imus_pub')

        # Initialize the two I2C buses
        self.i2c_buses = {
            2: I2C(0),  # /dev/i2c-0 for sensor 3 (Rear Left)
            3: I2C(1),  # /dev/i2c-1 for sensor 4 (Rear Right)
        }
        ...
        self.pub_2imus_rear = self.create_publisher(TwoIMU, '/two_imus_rear', 10)

        # Create a timer at ~250Hz 
        self.timer_period = 0.004  
        self.timer = self.create_timer(self.timer_period, self.publish_imus)

        self.get_logger().info("RearImusPublisher node started.")
    ...
    def publish_imus(self):
        ...
        self.pub_2imus_rear.publish(msg)
